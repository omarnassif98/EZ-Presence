...
def MakeQRCode(tID, className, sessionTitle):
    ...
    sessionID = db.reference('classes/' + tID + '/' + className + '/sessions').push({'in_session':True, 'title':sessionTitle, 'status':rosterStatus, 'location_data':{'required':False, 'longitude':0, 'latitude':0}}).key
    blob = storage_bucket.blob(tID + '/' + sessionID + '.png')
    img = BytesIO()
    qrcode.make("{0}|{1}|{2}".format(tID, className, sessionID)).save(img, format='png')
